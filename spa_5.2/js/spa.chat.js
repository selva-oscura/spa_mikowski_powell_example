...
spa.chat = (function(){
	...
	var 
		configMap={
			...
			slider_open_time 		: 500,
			slider_close_time 		: 500,
			slider_remove_time		: 1000,
			slider_opened_em 		: 18,
			slider_closed_em 		: 2,
		...
		onClickToggle, onClickRemove, 
		configModule, initModule,
		fadeSlider, removeSlider, 
		handleResize;
	// -------------------------END MODULE SCOPE VARIABLES--------------------------

	// ---------------------------BEGIN UTILITY METHODS-----------------------------
	// Begin utility method /getEmSize/
		// converts em display unit to pixels so we can use measurements in jQuery
	...
	// Example 	: spa.schat.setSliderPosition('closed');
	// Purpose 	: Move the chat slider to the requested position
	// Arguments: 
	// 	* position_type - enum('closed', 'opened', or 'hidden')
	// 	* callback - optional callback to be run at the end of slider animation.  The callback 
	// 	  receives a jQuery collection representing the slider div as its single argument.
	// Action 	:
	// This method moves the slider into the requested position.  If the requested position is 
	// the current position, it returns true without taking further action.
	// Returns 	:
	...
	onClickRemove = function(event){
		fadeSlider();
		setTimeout(removeSlider, configMap.slider_remove_time);
	}
	// -----------------------------END EVENT HANDLERS------------------------------
	...
	// 	* people_model - the people model object which provides methods to manage the list of people
	//    the model maintains
	...
	// Begin public method /initModule/
		// As with the shell, this routine has 3 parts:
	...
	//    It then initializes elements, events, & handlers to provide the user with a chat-room interface
	...
	initModule = function($append_target){
		...
		setJqueryMap();
		setPxSizes();
		// initialize the chat slider to default title and state
		jqueryMap.$toggle.prop('title', configMap.slider_closed_title);
		jqueryMap.$head.click(onClickToggle);
		jqueryMap.$closer.click(onClickRemove);
	...
	// Begin public method /fadeSlider/
	// Purpose 	:
	//	* Animate removal of slider
	//	* Call  removeSlider()
	// Arguments: none
	// Returns 	: none
	// Throws 	: none
	...
	fadeSlider=function(){
		if(jqueryMap.$slider){
			var animate_time = configMap.slider_remove_time;
			jqueryMap.$slider.animate(
				{opacity : 0,
					width: 0,
					height : 0},
	...
	removeSlider=function(){
		if(jqueryMap.$slider){
			...
			// remove DOM container; this also removes event bindings
